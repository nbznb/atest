...
// ============================================================================
// AI Technology Digest — Central Feed Generator
// ============================================================================
...
const BLOG_USER_AGENT = RSS_USER_AGENT;
...
async function fetchXContent(xSources, bearerToken, state, errors) {
  const results = [];
  const cutoff = new Date(Date.now() - TWEET_LOOKBACK_HOURS * 60 * 60 * 1000);

  // Batch lookup all user IDs (1 API call)
  const handles = xSources.map(a => a.handle);
  let userMap = {};
  ...
  // Fetch recent tweets per tracked source (max 3, exclude retweets/replies)
  for (const account of xSources) {
    ...
    try {
      ...
      // Filter out already-seen posts, cap at 3
      const newTweets = [];
...
function normalizeUstcUrl(href, baseUrl) {
  try {
...
function inferAnnouncementYear(month, day) {
  const now = new Date();
...
function extractTeachPublishedAt(fragment) {
  const dateMatch = fragment.match(/data-date="(\d{4}-\d{2}-\d{2})(?:\s+(\d{2}:\d{2}))?"/i);
  if (!dateMatch) return null;
  const time = dateMatch[2] || '00:00';
  return new Date(`${dateMatch[1]}T${time}:00+08:00`).toISOString();
}

function extractUstcIndexPublishedAt(fragment) {
  const dateMatch = fragment.match(/<font[^>]*>\s*(\d{2})-(\d{2})\s*<\/font>/i);
  if (!dateMatch) return null;
  return inferAnnouncementYear(Number(dateMatch[1]), Number(dateMatch[2]));
}

function extractNewsUstcPublishedAt(fragment) {
  const yearMonthMatch = fragment.match(/<div[^>]*class="[^"]*month[^"]*"[^>]*>\s*(\d{4})\.(\d{2})\s*<\/div>/i);
  const dayMatch = fragment.match(/<div[^>]*class="[^"]*day[^"]*"[^>]*>\s*(\d{1,2})\s*<\/div>/i);
  if (!yearMonthMatch || !dayMatch) return null;
  return new Date(`${yearMonthMatch[1]}-${yearMonthMatch[2]}-${dayMatch[1].padStart(2, '0')}T00:00:00+08:00`).toISOString();
}

function parseTeachUstcIndex(html, baseUrl) {
  const articles = [];
  const seen = new Set();
  const itemRegex = /<li[^>]*class="[^"]*post-[^"]*"[^>]*>([\s\S]*?)<\/li>/gi;
  let itemMatch;

  while ((itemMatch = itemRegex.exec(html)) !== null) {
    const block = itemMatch[1];
    const linkMatch = block.match(/<a href="([^"]*\/notice\/[a-z0-9-]+\/\d+\.html)"[^>]*>([\s\S]*?)<\/a>/i);
    if (!linkMatch) continue;

    const url = normalizeUstcUrl(linkMatch[1].trim(), baseUrl);
    ...
    articles.push({
      ...
      url,
      publishedAt: extractTeachPublishedAt(block),
      description: ''
    });
  }
...
function parseUstcAnnouncementsIndex(html, baseUrl) {
  const articles = [];
  const seen = new Set();
  ...
  while ((rowMatch = rowRegex.exec(html)) !== null) {
    ...
    const linkMatch = block.match(/<a href="((?:info\/\d+\/\d+\.htm)|(?:tzggcontent\.jsp\?[^"#]+))"[^>]*title="([^"]{4,200})"/i)
      || block.match(/<a href="((?:info\/\d+\/\d+\.htm)|(?:tzggcontent\.jsp\?[^"#]+))"[^>]*>([\s\S]*?)<\/a>/i);
    if (!linkMatch) continue;
    ...
    articles.push({
      title: decodeHtmlEntities(linkMatch[2].replace(/<[^>]+>/g, '').trim()),
      url,
      publishedAt: extractUstcIndexPublishedAt(block),
      description: ''
    });
  }
...
function parseNewsUstcIndex(html, baseUrl) {
  const articles = [];
  const seen = new Set();
  const itemRegex = /<div[^>]*class="[^"]*item[^"]*"[^>]*>([\s\S]*?)<\/div>\s*<\/div>\s*<\/div>/gi;
  let itemMatch;

  while ((itemMatch = itemRegex.exec(html)) !== null) {
    const block = itemMatch[1];
    const linkMatch = block.match(/<a href="((?:info\/\d+\/\d+\.htm)|(?:https?:\/\/news\.ustc\.edu\.cn\/info\/\d+\/\d+\.htm))"[^>]*>([\s\S]*?)<\/a>/i);
    if (!linkMatch) continue;
    ...
    const titleMatch = block.match(/<div[^>]*class="[^"]*info[^"]*"[^>]*>[\s\S]*?<a[^>]*>([\s\S]*?)<\/a>/i);
    articles.push({
      title: decodeHtmlEntities((titleMatch?.[1] || linkMatch[2]).replace(/<[^>]+>/g, '').trim()),
      url,
      publishedAt: extractNewsUstcPublishedAt(block),
      description: ''
    });
  }
...
function extractTeachUstcArticleContent(html) {
  const titleMatch = html.match(/<title>([\s\S]*?)<\/title>/i)
    || html.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i);
  const title = titleMatch
    ? decodeHtmlEntities(titleMatch[1].replace(/<[^>]+>/g, '').replace(/\s*:\s*中国科学技术大学教务处\s*$/i, '').trim())
    : '';

  const articleMatch = html.match(/<article[^>]*>([\s\S]*?)<\/article>/i)
    || html.match(/<main[^>]*class="[^"]*single[^"]*"[^>]*>([\s\S]*?)<\/main>/i);

  const content = articleMatch ? stripHtml(articleMatch[1]) : stripHtml(html);
  return { title, author: '', publishedAt: null, content };
}

function extractUstcAnnouncementContent(html) {
  const titleMatch = html.match(/<title>([\s\S]*?)<\/title>/i)
    || html.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i);
  const title = titleMatch
    ? decodeHtmlEntities(titleMatch[1].replace(/<[^>]+>/g, '').replace(/\s*-\s*中国科学技术大学\s*$/i, '').trim())
    : '';

  const bodyMatch = html.match(/<div[^>]*class="[^"]*v_news_content[^"]*"[^>]*>([\s\S]*?)<\/div>/i);
  const metaDescMatch = html.match(/<meta[^>]+name="description"[^>]+content="([^"]+)"/i);
  const content = bodyMatch ? stripHtml(bodyMatch[1]) : (metaDescMatch ? decodeHtmlEntities(metaDescMatch[1].trim()) : stripHtml(html));

  return { title, author: '', publishedAt: null, content };
}

function extractNewsUstcArticleContent(html) {
  const titleMatch = html.match(/<title>([\s\S]*?)<\/title>/i)
    || html.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i);
  const title = titleMatch
    ? decodeHtmlEntities(titleMatch[1].replace(/<[^>]+>/g, '').replace(/\s*-\s*中国科大新闻网\s*$/i, '').trim())
    : '';
...
async function fetchBlogContent(blogs, state, errors) {
  ...
  for (const blog of blogs) {
    ...
    try {
      ...
      } else if (blog.indexUrl.includes('teach.ustc.edu.cn')) {
        candidates = parseTeachUstcIndex(indexHtml, blog.indexUrl);
      } else if (blog.indexUrl.includes('ustc.edu.cn/tzgg')) {
        candidates = parseUstcAnnouncementsIndex(indexHtml, blog.indexUrl);
      } else if (blog.indexUrl.includes('news.ustc.edu.cn')) {
        candidates = parseNewsUstcIndex(indexHtml, blog.indexUrl);
      }
      ...
      for (const article of newArticles) {
        try {
          ...
          } else if (article.url.includes('teach.ustc.edu.cn/')) {
            extracted = extractTeachUstcArticleContent(articleHtml);
          } else if (article.url.includes('ustc.edu.cn/info/') || article.url.includes('ustc.edu.cn/tzggcontent.jsp')) {
            extracted = extractUstcAnnouncementContent(articleHtml);
          } else if (article.url.includes('news.ustc.edu.cn/info/')) {
            extracted = extractNewsUstcArticleContent(articleHtml);
          }
...
async function main() {
  ...
  // Fetch posts from tracked X sources
  if (runTweets) {
    console.error('Fetching X/Twitter content...');
    const xContent = await fetchXContent(sources.x_sources, xBearerToken, state, errors);
    console.error(`  Found ${xContent.length} sources with new posts`);
    ...
    const xFeed = {
      ...
      x: xContent,
      stats: { xSources: xContent.length, totalTweets },
    ...
    console.error(`  feed-x.json: ${xContent.length} sources, ${totalTweets} posts`);
  }
